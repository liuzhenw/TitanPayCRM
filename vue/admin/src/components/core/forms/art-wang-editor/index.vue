...
<script setup lang="ts">
  ...
  export interface ArtWangEditorExposed {
  ...
  type InsertFnType = (url: string, art?: string, href?: string) => void
  ...
  const props = withDefaults(defineProps<Props>(), {
    ...
    mode: 'default',
    placeholder: '请输入内容...',
    excludeKeys: () => ['fontFamily', 'group-video'],
    uploadConfig: () => ({
      maxFileSize: 3,
      container: 'products',
      savePath: 'html/images'
    })
  })
    ...
    MENU_CONF: {
      uploadImage: {
        async customUpload(file: File, insertFn: InsertFnType) {
          const maxFileSize = props.uploadConfig.maxFileSize * 1024 * 1024
          if (file.size > maxFileSize) {
            ElMessage.error(`不能上传大小超过 ${props.uploadConfig.maxFileSize} MB的图片`)
            return
          }
          ...
          const filePath = `${props.uploadConfig.savePath}/${fileName}`
          try {
            const res = await StoreageService.uploadFile({
              file,
              fileName: filePath,
              container: props.uploadConfig.container,
              useCompressor: true
            })
            insertFn(res.fileUrl, fileName)
          } catch (e) {
            console.error(e)
  ...
  const getToolbar = () => {
    ...
    return DomEditor.getToolbar(editorRef.value!)
  }
